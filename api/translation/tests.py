...
class AzureQueueWorkerTests(SimpleTestCase):
    ...
    @patch("api.translation.management.commands.run_ai_queue_worker.AzureQueueService")
    def test_worker_allows_existing_queue_on_startup(self, service_class):
        queue = Mock()
        queue.create_queue.side_effect = ResourceExistsError("Queue already exists")
        queue.receive_messages.return_value.by_page.return_value = iter([[]])
        service_class.return_value.is_configured = True
        service_class.return_value.queue_name = "meritlense-jobs"
        service_class.return_value.get_client.return_value = queue

        call_command("run_ai_queue_worker", once=True)

        queue.create_queue.assert_called_once()
